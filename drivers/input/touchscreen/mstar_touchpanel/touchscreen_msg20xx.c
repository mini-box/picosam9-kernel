...
static void mstar_ts_report_data(struct ms_ts_msg20xx_drvdata *tdata)
{
	...
	if (tdata->tsInfo.nKeyMode == TOUCHSCREEN_PREES || tdata->tsInfo.nKeyMode == TOUCH_RELEASE) {
		for (i = 0; i < tdata->tsInfo.nFingerNum; i++) {
			...
			input_report_abs(tdata->input, ABS_MT_POSITION_X,
					 (int)tdata->tsInfo.atPoint[i].Row);
			input_report_abs(tdata->input, ABS_MT_POSITION_Y,
					 (int)tdata->tsInfo.atPoint[i].Col);
...
{
	struct ms_ts_msg20xx_drvdata *tdata =
	    container_of(work, struct ms_ts_msg20xx_drvdata, work);
	u8 buf[8] = { 0 };
	int ret, i;
	u16 pointer_x[2] = { 0 };
	u16 pointer_y[2] = { 0 };
		...
		if (buf[0] == 0x52 && !checksum(buf, 8)) {
			...
			case TOUCHSCREEN_PREES:
				...
				TRACE("finger: %d,  p1[%d  %d]   p2[%d  %d]\n", finger,
				      pointer_x[0], pointer_y[0], pointer_x[1], pointer_y[1]);
				break;
...
static int ms_ts_msg20xx_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	tdata =
	    kzalloc(sizeof(*tdata) + MAX_TOUCH_FINGER * sizeof(*tdata->tsInfo.atPoint), GFP_KERNEL);
	if (!tdata)
		return -ENOMEM;
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, MS_TS_MSG20XX_X_MIN, MS_TS_MSG20XX_X_MAX,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, MS_TS_MSG20XX_Y_MIN, MS_TS_MSG20XX_Y_MAX,
			     0, 0);
	...
	ret =
	    request_irq(tdata->irq, ms_ts_msg20xx_isr, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,
			"ms_msg20xx", tdata);
	if (ret) {
	...
	dev =
	    device_create(i2c_dev_class, &client->adapter->dev,
			  MKDEV(I2C_MAJOR, client->adapter->nr), NULL, "msg20-%d",
			  client->adapter->nr);
	if (IS_ERR(dev)) {
		ret = PTR_ERR(dev);
